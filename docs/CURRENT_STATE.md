...
- Authentication is wired up (Authentication & Organization Onboarding milestone, Phases 1–2): `proxy.ts` + `lib/supabase/proxy.ts` refresh the session; `lib/auth/dal.ts` resolves identity/active-organization state; `lib/auth/actions.ts` implements sign in/up/out, password reset, organization selection, and organization bootstrap (via the `bootstrap_organization` RPC above); `app/login`, `app/onboarding`, `app/auth/*` are the corresponding routes. The `(app)` route group is gated on a resolved active organization; `/present` and `/demo/*` remain public/unauthenticated.
